...
public class RoleDescriptorDaoTest extends TestCase {
	...
	public void testSaveOrUpdateRoleDescriptor() {
		...
		RoleDescriptor roleDescriptorTmp = (RoleDescriptor) this.roleDescriptorDao
				.getHibernateTemplate().load(RoleDescriptor.class, id);
	...
	/**
	 * Test method for
	 * {@link woops2.hibernate.role.RoleDescriptorDao#getAllRoleDescriptor()}.
	 * 
	...
	public void testGetAllRoleDescriptor() {
		...
		this.roleDescriptorDao.getHibernateTemplate().saveOrUpdate(
				this.roleDescriptor);
	...
	/**
	 * Test method for
	 * {@link woops2.hibernate.role.RoleDescriptorDao#getRoleDescriptor()}.
	 * 
	 */
	public void testGetRoleDescriptor() {
		...
		RoleDescriptor roleDescriptorTmp = this.roleDescriptorDao
				.getRoleDescriptor(id);
		assertNotNull(roleDescriptorTmp);
		assertEquals("Name", roleDescriptorTmp.getName(), NAME);
		assertEquals("Description", roleDescriptorTmp.getDescription(),
				DESCRIPTION);
		assertEquals("Prefix", roleDescriptorTmp.getPrefix(), PREFIX);
		assertEquals("HasMultipleOccurences", roleDescriptorTmp
				.getHasMultipleOccurrences(), HAS_MULTIPLE_OCCURENCES);
		assertEquals("IsOptional", roleDescriptorTmp.getIsOptional(),
				IS_OPTIONAL);
	...
	/**
	 * Test method for
	 * {@link woops2.hibernate.role.RoleDescriptorDao#deleteRoleDescriptor()}.
	 * 
	 */
	public void testDeleteRoleDescriptor() {
		...
		RoleDescriptor roleDescriptorTmp = (RoleDescriptor) this.roleDescriptorDao
				.getHibernateTemplate().get(RoleDescriptor.class, id);
		assertNull(roleDescriptorTmp);

		// Test the method deleteRoleDescriptor with an roleDescriptor
