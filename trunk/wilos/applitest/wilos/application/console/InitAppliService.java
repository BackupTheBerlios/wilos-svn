...
public class InitAppliService {
	...
	public void projectInstanciation() {
		String s = processService.getProcessDao().getProcessFromGuid(
				"_9llsAQAvEdubGMceRDupFQ").getId();
		...
		String s2 = processService.getProcessDao().getProcessFromGuid(
				"_0uyGoMlgEdmt3adZL5Dmdw").getId();
	...
	public void initAppli() {
		...
		// ConcreteRoleDescriptorDao cs = (ConcreteRoleDescriptorDao)
		// factory.getBean("ConcreteRoleDescriptorDao");
		for (ConcreteRoleDescriptor crd : concreteRoleDescriptorDao
		...
		if (pa != null) {
			...
			for (ConcreteTaskDescriptor ctd : concreteTaskDescriptorDao
					.getAllConcreteTaskDescriptors()) {
				logger.debug("bababa");
				ConcreteTaskDescriptor ctd2 = concreteTaskDescriptorDao
						.getConcreteTaskDescriptor(ctd.getId());
				if (ctd2.getTaskDescriptor().getMainRole() != null) {
					logger.debug("appel");
					concreteTaskDescriptorService
							.affectedConcreteTaskDescriptor(ctd2, pa);
					logger.debug("sortie");
