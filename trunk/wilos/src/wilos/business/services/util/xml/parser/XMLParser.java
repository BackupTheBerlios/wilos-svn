...
public class XMLParser {
	...
	private static void setDependencyByActivity(Activity _act, Node _node) {
		...
		for (int i = 0; i < listOfTdNodes.getLength(); i++) {
			if (listOfTdNodes.item(i).getNodeName().equals(predecessor)) {
				...
				linkType = (listOfTdNodes.item(i).getAttributes().getNamedItem(
	 ...
	 * @param node
	 */
	private static void setDependencyByTaskDescriptor(TaskDescriptor _t,
			Node _node) {
		// search the predecessor
		...
		for (int i = 0; i < listOfTdNodes.getLength(); i++) {
			if (listOfTdNodes.item(i).getNodeName().equals(predecessor)) {
				...
				// process if there is a task for this task desriptor
				taskTobereturn = getTaskDescriptorById(taskDescriptorsList,
						idTask_pred);
				// if the task doesn't exist
	 ...
	 * @param bdeId
	 *            This methods gets the Bde From its List. The List and the id
	 *            of the BDe are determined using the Node given in parameter.
	 *            This methods fills the BreakdownElement given in parameter.
	 */
