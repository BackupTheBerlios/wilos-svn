 ...
 * Nicolas CASTEL <nic.castel@gmail.com>
 *
...
public class WizardServicesProxy {
	...
	public static void setIdentificationParamaters(String _aLogin,
			String _aPassword, String _anAddress) {
		WizardServicesProxy.login = _aLogin;
		WizardServicesProxy.password = _aPassword;
		WizardServicesProxy.address = cleanAddress(_anAddress);
	}

	private static String cleanAddress(String _anAddress) {
		String addressCleaned = _anAddress;
		if (addressCleaned.charAt(addressCleaned.length() - 1) == '/') {
			addressCleaned = addressCleaned.substring(0, addressCleaned
					.length() - 1);
		}
		...
		try {
			if (login.equalsIgnoreCase(testIHMLoginString)) {
				...
				WizardServicesService service = new WizardServicesService(
						new URL(address + ENDPOINT), new QName(URLWebService,
								nameWebService));
				WizardServices port = service.getWizardServicesPort();
				XStream xstream = new XStream();
				String result = port.getParticipant(login, password);
				/*
				 * File xstreamFile = new File ("xstream.xml"); PrintStream out =
				 * new PrintStream(new FileOutputStream(xstreamFile), false,
				 * "UTF-8"); out.println(result);
				 */
	...
	public static void startConcreteTaskDescriptor(String _theConcreteTaskId) {
		try {
			if (!login.equalsIgnoreCase(testIHMLoginString)) {
				WizardServicesService service = new WizardServicesService(
						new URL(address + ENDPOINT), new QName(URLWebService,
								nameWebService));
				WizardServices port = service.getWizardServicesPort();
				port.startConcreteTaskDescriptor(login, password,
						_theConcreteTaskId);
			}
		} catch (java.lang.Exception e) {
			new ExceptionManager(e);
	...
	public static byte[] getGuidanceAttachmentContent(String _idGuidance) {
			...
			if (!login.equalsIgnoreCase(testIHMLoginString)) {
				WizardServicesService service = new WizardServicesService(
						new URL(address + ENDPOINT), new QName(URLWebService,
								nameWebService));
				WizardServices port = service.getWizardServicesPort();
				fileContent = port.getGuidanceAttachment(login, password,
						_idGuidance);
			}
		} catch (java.lang.Exception e) {
			new ExceptionManager(e);
	...
	public static void stopConcreteTaskDescriptor(String _theConcreteTaskId) {
		try {
			if (!login.equalsIgnoreCase(testIHMLoginString)) {
				WizardServicesService service = new WizardServicesService(
						new URL(address + ENDPOINT), new QName(URLWebService,
								nameWebService));
				WizardServices port = service.getWizardServicesPort();
				port.stopConcreteTaskDescriptor(login, password,
						_theConcreteTaskId);
			}
		} catch (java.lang.Exception e) {
			new ExceptionManager(e);
	...
	public static void suspendConcreteTaskDescriptor(String _theConcreteTaskId) {
		try {
			if (!login.equalsIgnoreCase(testIHMLoginString)) {
				WizardServicesService service = new WizardServicesService(
						new URL(address + ENDPOINT), new QName(URLWebService,
								nameWebService));
				WizardServices port = service.getWizardServicesPort();
				port.suspendConcreteTaskDescriptor(login, password,
						_theConcreteTaskId);
			}
		} catch (java.lang.Exception e) {
			new ExceptionManager(e);
	...
	public static void resumeConcreteTaskDescriptor(String _theConcreteTaskId) {
		try {
			if (!login.equalsIgnoreCase(testIHMLoginString)) {
				WizardServicesService service = new WizardServicesService(
						new URL(address + ENDPOINT), new QName(URLWebService,
								nameWebService));
				WizardServices port = service.getWizardServicesPort();
				port.resumeConcreteTaskDescriptor(login, password,
						_theConcreteTaskId);
			}
		} catch (java.lang.Exception e) {
			new ExceptionManager(e);
	...
	public static void setRemainingTimeByTask(String _taskGuid, float _newTime) {
		try {
			if (!login.equalsIgnoreCase(testIHMLoginString)) {
				WizardServicesService service = new WizardServicesService(
						new URL(address + ENDPOINT), new QName(URLWebService,
								nameWebService));
				WizardServices port = service.getWizardServicesPort();
				port.setRemainingTimeByTask(login, password, _taskGuid,
						_newTime);
			}
		} catch (java.lang.Exception e) {
			new ExceptionManager(e);
	...
	public static void setAccomplishedTimeByTask(String _taskGuid,
			float _newTime) {
		try {
			if (!login.equalsIgnoreCase(testIHMLoginString)) {
				WizardServicesService service = new WizardServicesService(
						new URL(address + ENDPOINT), new QName(URLWebService,
								nameWebService));
				WizardServices port = service.getWizardServicesPort();
				port.setAccomplishedTimeByTask(login, password, _taskGuid,
						_newTime);
			}
		} catch (java.lang.Exception e) {
			new ExceptionManager(e);
	...
	private static Participant getParticipantExample() {
		...
		aTmpTask
				.setDescription("Un grand moment d'amour tout le monde sait tres bien qu'un developper aime son programme de toutes maniere ne pas aimer son programme est un crime. <br> Aimer ou ne pas aimer est un programme telle est la questin apres tout est ce que le developpeur ne laisse pas une partie de son ame dans son programme. <br> le developpeur ne fait qu'un avec son logiciel il entretient avec lui une union charnelle");
		...
		s1.setName("Correct");
		s1
				.setDescription("Does the requirement correctly specify a true need, desire, or obligation?");
		...
		s4
				.setDescription("Is the requirement in conflict with other requirements?");
		...
		aTmpTask.setGuid("42");
		aTmpTask
				.setPresentationName("Conceptualiser les concepts du programme");
		aTmpRole.addPrimaryTask(aTmpTask);
